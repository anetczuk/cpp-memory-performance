...
raw_file_prefix=./data/raw_data_vector_mp_core
plot_file_prefix=./data/plot_data_vector_mp_core
...
if [ $run_benchmark -ne 0 ]; then
	...
	for i in $(seq 1 $cores_num); do
	   ...
	   ./benchmark_vector_single > $out_file &
	   pids[${i}]=$!
	done  
...
echo "Plotting"
gnuplot -p -e 'set title "multiprocessed std::vector access times"; filenames = "'"$files_string"'"' plot_config_multi.gnu
